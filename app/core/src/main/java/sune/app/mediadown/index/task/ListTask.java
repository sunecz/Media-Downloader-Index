...
import sune.app.mediadown.index.event.Event;
import sune.app.mediadown.index.event.EventType;
...
public abstract class ListTask<T> extends Task {
	...
	public <W extends T, V> void forwardAdd(ListTask<V> other, Function<W, V> transform) {
		forward(other, ListTaskEvent.ADD, ListTask::add, (p) -> transform.apply(Utils.cast(p.b)));
	}
	...
	public <W extends T, V> void forwardAdd(Collection<V> collection, Function<W, V> transform) {
		addEventListener(ListTaskEvent.ADD, (p) -> collection.add(transform.apply(Utils.cast(p.b))));
	}
	...
	public static final class ListTaskEvent implements EventType {
		...
		public static Event<ListTaskEvent, ?>[] values() {
			if(values == null) {
