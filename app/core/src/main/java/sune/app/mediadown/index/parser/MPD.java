package sune.app.mediadown.index.parser;
...
import sune.app.mediadown.index.media.*;
...
import sune.app.mediadown.index.segment.RemoteFileSegment;
import sune.app.mediadown.index.segment.RemoteFileSegmentsHolder;
import sune.app.mediadown.index.util.CheckedFunction;
...
public final class MPD {
	...
	private static double requireValidTime(double value) {
		if(value < 0.0)
			throw new IllegalArgumentException();
		return value;
	}
	
	private static int requireValidCount(int value) {
		if(value < 0)
			throw new IllegalArgumentException();
		return value;
	}
	
	private static long requireValidTime(long value) {
		if(value < 0L)
			throw new IllegalArgumentException();
		return value;
	}
	
	private static int requireIntPositive(int value) {
		if(value <= 0)
			throw new IllegalArgumentException();
		return value;
	}
	...
	private static Map<String, String> elementAttributesToMap(Attributes attributes) {
		return Utils.stream(attributes.iterator()).collect(Collectors.toMap(Attribute::getKey, Attribute::getValue));
	}
	
	private static CheckedFunction<URI, Response.OfStream> streamResolver(Request request) {
		return ((uri) -> Web.requestStream(request.ofURI(uri)));
	}
	
	public static List<MPDFile> parse(Request request) throws Exception {
		return new MPDReader(request.uri(), streamResolver(request)).read();
	}
	
	public static List<MPDFile> parse(String uri, String content) throws Exception {
		return new MPDReader(Net.uri(uri), content).read();
	}
	
	public static List<MPDCombinedFile> reduce(List<MPDFile> files) {
	...
	private static final class Segment {
		...
		public static Segment parse(Element element) {
			long time = -1L;
			long duration = -1L;
			int count = 1;
	...
	private static final class SegmentTimeline {
		...
		public static SegmentTimeline parse(Element element) {
	...
	private static final class SegmentTemplate {
		...
		public static SegmentTemplate parse(Element element) {
			if(!Objects.requireNonNull(element).nodeName().equalsIgnoreCase(NODE_NAME))
				throw new IllegalArgumentException();
		...
		public SegmentTimeline timeline() {
			return timeline;
		}
	}
	...
	private static final class Representation {
		...
		public static Representation parse(Element element) {
			if(!Objects.requireNonNull(element).nodeName().equalsIgnoreCase(NODE_NAME))
				throw new IllegalArgumentException();
		...
		public MPDFile apply(URI baseURI, MediaFormat format, SegmentTemplate template,
	...
	private static final class MPDFileConstructor {
		...
		private void prepareFormatMap() {
			formatMap.put("RepresentationID", representation.id());
		}
		
		private static Object castToFormatArg(String formatArg, String value) {
		...
		private String format(MatchResult result) {
		...
		private void addSegment(String templateURI, Segment segment) throws Exception {
		...
		private void addSegment(String templateURI, double duration, boolean isInitialization) throws Exception {
		...
		public MPDFile construct(MediaFormat format) throws Exception {
	...
	private static final class AdaptationSet {
		...
		public static AdaptationSet parse(Element element) {
			if(!Objects.requireNonNull(element).nodeName().equalsIgnoreCase(NODE_NAME))
				throw new IllegalArgumentException();
		...
		public List<MPDFile> process(URI baseURI) throws Exception {
	...
	private static final class MPDReader {
		...
		private void throwExceptionInvalid(String message) throws IOException {
			throw new IOException("Invalid MPD file" + (message != null ? ": " + message : ""));
		}
		
		private Document responseDocument(InputStream stream, URI baseURI)
				throws IOException {
			return Jsoup.parse(stream, Shared.CHARSET.name(), baseURI.toString());
		}
		
		private List<MPDFile> read(URI baseURI, Document document) throws Exception {
			List<MPDFile> files = new ArrayList<>();
		...
		public List<MPDFile> read() throws Exception {
	...
	public static final class ContentProtection {
		...
		private static final String NODE_NAME = "ContentProtection";
		...
		public static ContentProtection parse(Elements elements) {
	...
	public static final class MPDSegment {
		...
		private MPDSegment(URI uri, double duration, double startTime) {
			this.uri = Objects.requireNonNull(uri);
			this.duration = requireValidTime(duration);
	...
	public static final class MPDFile {
		...
		private MPDFile(List<MPDSegment> segments, MediaFormat format, MediaResolution resolution, double duration,
                        ContentProtection protection, Map<String, String> attributes) {
		...
		public List<MPDSegment> segments() {
			return segments;
		}
	...
	public static final class MPDCombinedFile {
		...
		private MPDCombinedFile(MPDFile video, MPDFile audio) {
			this.video = Objects.requireNonNull(video);
			this.audio = Objects.requireNonNull(audio);
		}
		
		public MPDFile video() {
			return video;
		}
		
		public MPDFile audio() {
			return audio;
		}
		
		public List<MPDFile> files() {
			if(files == null) {
