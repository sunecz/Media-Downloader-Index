...
import sune.app.mediadown.index.concurrent.StateMutex;
import sune.app.mediadown.index.event.*;
import sune.app.mediadown.index.util.*;
...
public abstract class Task implements EventBindable<EventType>, HasTaskState {
	...
	public static final class TaskEvent implements EventType {
		...
		public static Event<TaskEvent, ?>[] values() {
			if(values == null) {
				values = Utils.array(BEGIN, RESUME, END, ERROR);
			}
