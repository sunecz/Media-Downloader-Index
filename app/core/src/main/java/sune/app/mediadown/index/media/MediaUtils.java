...
import sune.app.mediadown.index.media.MediaQuality.QualityValue;
import sune.app.mediadown.index.media.MediaQuality.VideoQualityValue;
...
import sune.app.mediadown.index.parser.M3U;
import sune.app.mediadown.index.parser.MPD;
import sune.app.mediadown.index.parser.M3U.M3UCombinedFile;
import sune.app.mediadown.index.parser.M3U.M3UFile;
import sune.app.mediadown.index.parser.MPD.ContentProtection;
import sune.app.mediadown.index.parser.MPD.MPDCombinedFile;
import sune.app.mediadown.index.parser.MPD.MPDFile;
import sune.app.mediadown.index.segment.FileSegment;
import sune.app.mediadown.index.util.Opt;
import sune.app.mediadown.index.util.Ref;
import sune.app.mediadown.index.util.Utils;
import sune.app.mediadown.index.util.Opt.OptCondition;
import sune.app.mediadown.index.util.Opt.OptMapper;
import sune.app.mediadown.index.util.Utils.Ignore;
...
public final class MediaUtils {
	...
	private static MediaMetadata.Builder mergeMetadataBuilder(MediaMetadata a, MediaMetadata b) {
	...
	public static List<Media.Builder<?, ?>> createMediaBuilders(MediaSource source, URI uri, URI sourceURI,
	...
	public static List<Media> createMedia(MediaSource source, URI uri, URI sourceURI, String title,
	...
	public static String mediaTitle(String programName, int numSeason, int numEpisode, String episodeName) {
	...
	public static String mediaTitle(String programName, String numSeason, String numEpisode, String episodeName) {
		return mediaTitle(programName, numSeason, numEpisode, episodeName, false);
	}
	...
	public static String mediaTitle(String programName, String numSeason, String numEpisode, String episodeName,
		...
		return programName + " - S" + numSeason + "E" + numEpisode + " - " + episodeName; 
	}
	
	public static boolean isSegmentedMedia(Media media) {
		return streamFilterRecursive(media, OptCondition.of(Media::isSegmented))
					.anyMatch(OptCondition.ofTrue());
	}
	
	public static List<Media> solids(Media media) {
		return filterRecursive(media, OptCondition.ofAll(Media::isSolid));
	}
	
	public static Stream<Media> streamFilterDirect(Media media, OptCondition<Media> condition) {
		return streamFilter(media, MediaContainer::direct, condition);
	}
	
	public static Stream<Media> streamFilterRecursive(Media media, OptCondition<Media> condition) {
		return streamFilter(media, MediaContainer::recursive, condition);
	}
	
	public static Stream<Media> streamFilter(
	...
	public static List<Media> filterDirect(Media media, OptCondition<Media> condition) {
		return filter(media, MediaContainer::direct, condition);
	}
	
	public static List<Media> filterRecursive(Media media, OptCondition<Media> condition) {
		return filter(media, MediaContainer::recursive, condition);
	}
	
	public static List<Media> filter(
            Media media, Function<MediaContainer, MediaAccessor> accessorMapper,
            OptCondition<Media> condition
    ) {
	...
	public static MediaQuality estimateMediaQualityFromBandwidth(int bandwidth) {
		return MediaQualityEstimator.fromBandwidth(bandwidth);
	}
	
	public static double estimateTotalSize(Media media) {
		return TotalSizeEstimator.estimate(media);
	}
	
	public static MediaMetadata mergeMetadata(MediaMetadata a, MediaMetadata b) {
	...
	private static final class MediaQualityEstimator {
		...
		private static double bandwidthToBitRateMbps(int bandwidth) {
			return bandwidth / 1024.0 / 1024.0;
		}
		
		private static int approximateVideoHeightFromBandwidth(int bandwidth) {
		...
		public static MediaQuality fromBandwidth(int bandwidth) {
	...
	private static final class TotalSizeEstimator {
		...
		private static int bitRateMbpsToBandwidth(double bitRate) {
			return Math.max(0, (int) Math.ceil(bitRate * 1024.0 * 1024.0));
		}
		
		// Returns values in Mbps
		private static int approximateBandwidthFromVideoHeight(int height) {
		...
		// Returns values in Mbps
		private static int approximateBandwidthFromAudioSampleRate(int sampleRate) {
			// Approximation based on the approximation table [2].
			return bitRateMbpsToBandwidth(sampleRate / 1000.0 * 32.0 / 1024.0);
		}
		
		private static int approximateBandwidth(VideoMedia video) {
		...
		private static int approximateBandwidth(AudioMedia audio) {
			return approximateBandwidthFromAudioSampleRate(audio.sampleRate());
		}
		
		private static double fromBandwidth(int bandwidth, double duration) {
			if(duration < 0.0) {
		...
		public static double estimate(VideoMedia video) {
		...
		public static double estimate(AudioMedia audio) {
		...
		public static double estimate(Media media) {
	...
	private static final class SimpleM3U8FormatParser extends Parser.M3U8FormatParser {
		...
		@Override
		protected Media.Builder<?, ?> map(Parser.FormatParserData<M3UCombinedFile> parserData) {
	...
	private static final class SimpleDASHFormatParser extends Parser.DASHFormatParser {
		...
		@Override
		protected Media.Builder<?, ?> map(Parser.FormatParserData<MPDCombinedFile> parserData) {
	...
	private static final class Parser {
		...
		private static boolean isNotUnknownFormat(MediaFormat format) {
			return format != null && !format.is(MediaFormat.UNKNOWN);
		}
		
		private static MediaFormat contentType(HttpHeaders headers) {
		...
		private static MediaFormat contentTypeFromUri(URI uri) {
			return MediaFormat.fromPath(uri.getPath());
		}
		
		private List<Media.Builder<?, ?>> parseFormat(URI uri, MediaFormat format, Request request,
		...
		public List<Media.Builder<?, ?>> parse(URI uri, URI sourceURI, Map<String, Object> data)
				throws Exception {
			Request request = Request.of(uri).GET();
			try(Response.OfVoid response = Web.peek(request)) {
		...
		public <T> Parser format(MediaFormat format, FormatParser parser) {
			parsers.put(Objects.requireNonNull(format), Objects.requireNonNull(parser));
			return this;
		}
		
		public Parser defaultParser(FormatParser parser) {
