...
public final class M3U {
	...
	private static CheckedFunction<URI, Response.OfStream> streamResolver() {
		return ((uri) -> Web.requestStream(Request.of(uri).GET()));
	}
	
	private static List<M3UCombinedFile> build(M3UReaderResult result) {
	...
	public static List<M3UCombinedFile> parse(Request request) throws Exception {
	...
	public static List<M3UCombinedFile> parse(String uri, String content) throws Exception {
	...
	public static final class M3USegment extends RemoteFileSegment {
		...
		private M3USegment(int index, URI uri, double duration, String dateTime) {
	...
	public static final class M3UFile {
		...
		private M3UFile(M3UFileType type, URI uri, RemoteFileSegmentsHolder segmentsHolder, double duration,
		...
		private static MediaProtection mediaProtectionOf(M3UKey key) {
	...
	public static final class M3UCombinedFile {
		...
		public static M3UCombinedFile ofCombined(M3UFile videoAndAudio) {
			return new M3UCombinedFile(Objects.requireNonNull(videoAndAudio), null, null);
		}
		
		public static M3UCombinedFile ofCombined(M3UFile videoAndAudio, List<M3UFile> subtitles) {
			return new M3UCombinedFile(Objects.requireNonNull(videoAndAudio), null, Objects.requireNonNull(subtitles));
		}
		
		public static M3UCombinedFile ofSeparate(M3UFile video, M3UFile audio) {
			return new M3UCombinedFile(Objects.requireNonNull(video), Objects.requireNonNull(audio), null);
		}
		
		public static M3UCombinedFile ofSeparate(M3UFile video, M3UFile audio, List<M3UFile> subtitles) {
	...
	protected static final class M3UReader implements AutoCloseable {
		...
		private Pair<String, String> parseMetaLine(String line) {
		...
		private void parseSegmentInfo(String value) throws IOException {
		...
		private void parseSegmentDateTime(String value) throws IOException {
			segmentBuilder.dateTime(value);
		}
		
		private void parseStreamInfo(String value) throws IOException {
		...
		private Map<String, String> parseAttributeList(String value) {
		...
		private void parseMedia(String value) throws Exception {
			Map<String, String> attrs = parseAttributeList(value);
		...
		private void updateMetaData(String name, String value) throws Exception {
			switch(name) {
		...
		private List<M3UFileBuilder> readStreamInfo(String uri) throws Exception {
			URI resolvedURI = resolveURI(uri);
			URI resolvedBaseURI = Net.isRelativeURI(uri) ? resolveURI(uri) : Net.baseURI(Net.uri(uri));
		...
		public M3UReaderResult read() throws Exception {
