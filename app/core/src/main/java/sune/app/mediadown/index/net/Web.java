...
import sune.app.mediadown.index.util.Range;
...
public final class Web {
	...
	private static HttpClient newDefaultHttpClientWithRedirect() {
		return newHttpClientBuilder().followRedirects(Redirect.NORMAL).build();
	}

	private static HttpClient newDefaultHttpClientNoRedirect() {
		return newHttpClientBuilder().followRedirects(Redirect.NEVER).build();
	}

	private static HttpClient newProxyHttpClientWithRedirect(Proxy proxy) {
		return newHttpClientBuilder().followRedirects(Redirect.NORMAL).proxy(proxy.selector()).build();
	}

	private static HttpClient newProxyHttpClientNoRedirect(Proxy proxy) {
	...
	private static HttpRequest.Builder newHttpRequestBuilder() {
		return HttpRequest.newBuilder()
					.setHeader("User-Agent", USER_AGENT)
					.timeout(defaultReadTimeout);
	}
	
	private static Duration checkTimeout(Duration timeout) {
	...
	private static HttpClient httpClientFor(Request request) {
	...
	private static boolean isInternallyRetryableError(Throwable cause) {
	...
	private static boolean isExternallyRetryableError(Throwable cause) {
		return cause instanceof SocketTimeoutException;
	}
	
	private static void waitInternalRetry(int retry) throws InterruptedException {
		int millis = Math.min(
	...
	private static <T, R extends Response> R doRequest(
	...
	private static Regex regexContentRange() {
	...
	public static void defaultConnectTimeout(Duration timeout) {
		defaultConnectTimeout = checkTimeout(timeout);
	}
	
	public static void defaultReadTimeout(Duration timeout) {
	...
	public static Response.OfString request(Request request) throws Exception {
		return doRequest(request, Response.OfString::new, BodyHandlers.ofString(CHARSET));
	}
	
	public static Response.OfStream requestStream(Request request) throws Exception {
		return doRequest(request, Response.OfStream::new, BodyHandlers.ofInputStream());
	}
	
	public static Response.OfVoid peek(Request request) throws Exception {
		return doRequest(request.toHEAD(), Response.OfVoid::new, BodyHandlers.discarding());
	}
	
	public static long size(Request request) throws Exception {
		return size(peek(request));
	}
	
	public static long size(Response response) throws Exception {
		return response.statusCode() != 200 ? UNKNOWN_SIZE : size(response.headers());
	}
	
	public static long size(HttpHeaders headers) {
	...
	private static final class Internal {
		...
		public static Version getHttpVersion(Request request) {
			URI normalizedUri = normalizeForLookup(request.uri());
	...
	private static interface Client {
		...
		public static final class OfProxy implements Client {
			...
			private HttpClient newWithRedirect() {
			...
			private HttpClient newNoRedirect() {
				Proxy proxy = refProxy.get();
	...
	public static abstract class Request {
		...
		protected static class POST extends Request {
			...
			protected POST(Builder builder, HttpRequest.BodyPublisher body, String contentType) {
				super("POST", builder);
		...
		public static class Builder {
			...
			protected static <T> List<T> merge(List<T> list, Collection<T> values) {
				list.addAll(values);
				return list;
			}
			...
			public Request POST(HttpRequest.BodyPublisher body, String contentType) {
	...
	public static final class Proxy {
		...
		public static Proxy of(String scheme, InetSocketAddress address, String authorization) {
	...
	public static final class Headers {
		...
		private static BiPredicate<String, String> newFilter() {
			return (a, b) -> true;
		}
		
		private static BiPredicate<String, String> noFilter() {
			return filter.value();
		}
		
		private static HttpHeaders newEmpty() {
		...
		public static HttpHeaders ofMap(Map<String, List<String>> headers) {
			return HttpHeaders.of(headers, noFilter());
		}
		
		public static HttpHeaders ofSingleMap(Map<String, String> headers) {
			return ofMap(
		...
		public static HttpHeaders ofSingle(Object... values) {
		...
		public static HttpHeaders ofString(String string) {
		...
		private static final class Parser {
			...
			private static boolean isNameDelimiter(int c) {
				return c == CHAR_ITEM_DELIMITER1 || c == CHAR_ITEM_DELIMITER2 || c == CHAR_NAME_DELIMITER;
			}
			
			private static boolean isValueDelimiter(int c) {
				return c == CHAR_ITEM_DELIMITER1 || c == CHAR_ITEM_DELIMITER2;
			}
			
			private static List<String> merge(String name, String value, List<String> existing) {
				if(existing == null) {
			...
			private void readString(StringBuilder str, Predicate<Integer> isDelimiter) {
			...
			private void readName(StringBuilder str) {
				readString(str, Parser::isNameDelimiter);
			}
			
			private void readValue(StringBuilder str) {
	...
	public static final class Cookies {
		...
		public static void add(URI uri, HttpCookie cookie, HttpCookie... more) {
			add(uri,
				more.length > 0
		...
		public static void add(URI uri, List<HttpCookie> cookies) {
			CookieStore store = cookieManager().getCookieStore();
			cookies.forEach((cookie) -> store.add(uri, cookie));
		}
		
		public static void set(URI uri, HttpCookie cookie, HttpCookie... more) {
			clear(uri);
			add(uri, cookie, more);
		}
		
		public static void set(URI uri, List<HttpCookie> cookies) {
			clear(uri);
			add(uri, cookies);
		}
		
		public static List<HttpCookie> get(URI uri) {
			return cookieManager().getCookieStore().get(uri);
		}
		
		public static HttpCookie stringToCookie(String string) {
