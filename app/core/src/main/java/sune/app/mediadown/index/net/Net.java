...
public final class Net {
	...
	private static String uriSanitize(String uri) {
		return uri.replace("|", "%7C");
	}
	
	private static void queryConstruct(StringBuilder builder, QueryArgument argument, String namePrefix) {
	...
	private static void queryMap(Map<String, Object> map, QueryArgument argument, String namePrefix) {
	...
	public static String uriConcat(String... parts) {
		return Regex.of("([^:])//+").replaceAll(Utils.join("/", parts), "$1/");
	}
	
	public static String uriFix(String uri) {
		return uri.startsWith("//") ? "https:" + uri : uri;
	}
	
	public static URI uriBasename(URI uri) {
		return uri(Utils.afterLast(uri.getRawPath(), "/"));
	}
	
	public static URI uriBasename(String uri) {
		return uriBasename(uri(uri));
	}
	
	public static URI uriDirname(URI uri) {
		return uri.getRawPath().endsWith("/") ? uri.resolve("..") : uri.resolve(".");
	}
	
	public static URI uriDirname(String uri) {
	...
	public static InputStream stream(URI uri, Duration timeout) throws IOException {
		return stream(url(uri), timeout);
	}
	
	public static InputStream stream(URL url, Duration timeout) throws IOException {
		URLConnection connection = url.openConnection();
		connection.setConnectTimeout((int) timeout.toMillis());
		return connection.getInputStream();
	}
	
	public static URI resolve(URI uri, URI other) {
		return uri.resolve(other);
	}
	
	public static URI resolve(URI uri, String other) {
		return uri.resolve(uri(other));
	}
	
	public static String queryConstruct(QueryArgument argument) {
		StringBuilder builder = new StringBuilder();
		queryConstruct(builder, argument, null);
		return builder.toString();
	}
	
	public static String queryConstruct(List<QueryArgument> arguments) {
		StringBuilder builder = new StringBuilder();
	...
	public static QueryArgument queryDestruct(URI uri) {
		return queryDestruct(uri.getRawQuery());
	}
	
	public static QueryArgument queryDestruct(String uri) {
	...
	public static QueryArgument createQuery(Map<String, Object> args) {
		QueryArgument.Builder root = QueryArgument.Builder.ofRoot();
	...
	public static QueryArgument createQuery(Object... args) {
		return createQuery(Utils.stringKeyMap(args));
	}
	
	public static String queryString(Map<String, Object> args) {
	...
	public static Map<String, Object> queryMap(QueryArgument argument) {
		Map<String, Object> map = new LinkedHashMap<>();
		queryMap(map, argument, null);
		return map;
	}
	
	public static Map<String, Object> queryMap(List<QueryArgument> arguments) {
		Map<String, Object> map = new LinkedHashMap<>();
	...
	public interface QueryArgument {
		...
		static QueryArgument ofValue(String name, String value) {
			return new OfValue(name, value);
		}
		
		final class OfValue implements QueryArgument {
			...
			private static void notAnArray() {
		...
		final class OfArray implements QueryArgument {
			...
			private QueryArgument firstArgument() {
			...
			private QueryArgument get(String name) {
				return values.get(name);
			}
			
			private QueryArgument get(int index) {
				return values.get(Integer.toString(index));
			}
			
			private List<QueryArgument> listOfArguments() {
				return List.copyOf(values.values());
			}
			
			private Map<String, QueryArgument> mapOfArguments() {
			...
			private <T> String valueOrDefault(Function<T, QueryArgument> function, T input, String defaultValue) {
