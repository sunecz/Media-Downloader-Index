...
import sune.app.mediadown.index.concurrent.VarLoader;
import sune.app.mediadown.index.media.MediaFormat;
...
public final class Utils {
	...
	public static String format(String string, Map<String, Object> values) {
		return format(defaultLocale(), string, values);
	}
	
	public static String format(Locale locale, String string, Object... values) {
		return format(locale, string, stringKeyMap(values));
	}
	
	public static String format(Locale locale, String string, Map<String, Object> values) {
	...
	public static double convertToSeconds(String string) {
		double val  = 0.0;
	...
	public static double convertToMilliseconds(String string) {
		return convertToSeconds(string) * 1e3;
	}
	
	public static void combineFiles(List<Path> paths, Path dest) {
		try {
	...
	// not using any built-in function and ignoring all other characters that
	// are not defined as digits.
	private static int extractIntOrDefault(String string, Integer defaultValue) {
	...
	public static <T> T[] arrayOfSize(Class<? extends T> clazz, int size) {
		@SuppressWarnings("unchecked")
	...
	public static Map<String, Object> stringKeyMap(Object... data) {
	...
	public static String addFormatExtension(String name, MediaFormat format) {
		return name + (!format.fileExtensions().isEmpty() ? '.' + format.fileExtensions().get(0) : "");
	}
	
	public static String num2string(double value, int decimals) {
	...
	private static Decoder base64URLDecoder() {
	...
	public static byte[] base64DecodeRaw(String string) {
	...
	public static byte[] base64URLDecodeRaw(String string) {
		return base64URLDecoder().decode(string.getBytes(CHARSET));
	}
	
	public static String base64URLDecode(String string) {
	...
	private static Encoder base64URLEncoder() {
	...
	public static byte[] base64EncodeRaw(String string) {
	...
	public static byte[] base64URLEncodeRaw(String string) {
	...
	public static byte[] hexStringToArray(String string) {
	...
	public static Pair<Integer, Integer> stringBetween(String string, int chOpen, int chClose) {
		return stringBetween(string, chOpen, chClose, 0, string.length());
	}
	
	public static Pair<Integer, Integer> stringBetween(String string, int chOpen, int chClose, int start, int end) {
		int count = 0;
	...
	public static Pair<Integer, Integer> stringBetweenReverse(String string, int chOpen, int chClose) {
		return stringBetweenReverse(string, chOpen, chClose, codePointAlign(string, string.length() - 1), -1);
	}
	
	public static Pair<Integer, Integer> stringBetweenReverse(String string, int chOpen, int chClose, int start, int end) {
		int count = 0;
	...
	public static String bracketSubstring(String string) {
		return bracketSubstring(string, '(', ')', false);
	}
	
	public static String bracketSubstring(String string, boolean reverse) {
		return bracketSubstring(string, '(', ')', reverse);
	}
	
	public static String bracketSubstring(String string, int chOpen, int chClose) {
		return bracketSubstring(string, chOpen, chClose, false);
	}
	
	public static String bracketSubstring(String string, int chOpen, int chClose, boolean reverse) {
	...
	public static String bracketSubstring(String string, int chOpen, int chClose, boolean reverse, int start, int end) {
		Pair<Integer, Integer> range
			= reverse
	...
	public static String substringTo(String string, int ch) {
		int index = string.indexOf(ch);
		return index >= 0 ? string.substring(0, index) : string;
	}
	
	public static String replaceUnicodeEscapeSequences(String string) {
		return UnicodeEscapeSequence.replace(string);
	}
	
	public static String prefixUnicodeEscapeSequences(String string, String prefix) {
		return UnicodeEscapeSequence.prefix(string, prefix);
	}
	
	public static void unslash(StringBuilder input, StringBuilder output) {
		final int len = input.length();
		unslash(input, output, len);
	}
	
	public static void unslash(StringBuilder input, StringBuilder output, final int len) {
		int end = 0;
	...
	public static void unslash(String input, StringBuilder output, final int len) {
		int end = 0;
		boolean escaped = false;
	...
	public static String unslash(String string) {
	...
	public static int backTill(String string, int ch, int from) {
		return backTill(string, ch, from, 1);
	}
	
	public static int backTill(String string, int ch, int from, int count) {
	...
	public static String backTillString(String string, int ch, int from) {
		return backTillString(string, ch, from, 1);
	}
	
	public static String backTillString(String string, int ch, int from, int count) {
	...
	public static int charCount(String string, int index) {
	...
	public static int codePointAlign(String string, int index) {
		int c = string.codePointAt(index);
	...
	public static String trimEnd(String string, int ch) {
	...
	public static int indexOf(String string, String substring, int iter) {
		if((iter <= 0)) return -1;
		int start = 0, index;
		do {
	...
	public static String before(String string, int index, int offset) {
		return index >= 0 ? string.substring(0, index + offset) : string;
	}
	
	public static String beforeFirst(String string, String what) {
		return before(string, string.indexOf(what), 0);
	}
	
	public static String beforeLast(String string, String what) {
	...
	public static String after(String string, int index, int offset) {
		return index >= 0 ? string.substring(index + offset) : string;
	}
	
	public static String afterFirst(String string, String what) {
		return after(string, string.indexOf(what), what.length());
	}
	
	public static String afterLast(String string, String what) {
	...
	public static <T> List<T> lastItems(Collection<T> coll, int count) {
	...
	public static long getUnixTime(String zone) {
	...
	public static int[] intArray(Integer[] a) {
		int   l = a.length;
		int[] b = new int[l];
		for(int i = 0; i < l; ++i)
			b[i] = a[i];
	...
	public static <T, E> E getItemFromArray(T item, E[] array, Function<E, T> converter) {
		E e; T t;
	...
	@SafeVarargs
	public static <T> T[] array(Class<? extends T> clazz, T... items) {
		@SuppressWarnings("unchecked")
	...
	public static <T> T loopTo(Iterator<T> iterator, int index) {
		int current = 0;
		T item;
	...
	public static boolean startsWithIgnoreCase(String string, String value) {
	...
	public static Class<?> getClassNoException(String className) {
		try { return Class.forName(className); } catch(ClassNotFoundException ex) { return null; }
	}
	
	@SafeVarargs
	public static <T> T[] arrayPrepend(T item, T... items) {
		@SuppressWarnings("unchecked")
	...
	@SafeVarargs
	public static <T> T[] arrayAppend(T item, T... items) {
		@SuppressWarnings("unchecked")
		T[] array = (T[]) Array.newInstance(items.getClass().getComponentType(), items.length + 1);
	...
	@SafeVarargs
	public static <T> List<T> toModifiableList(T... items) {
	...
	@SafeVarargs
	public static <T> String join(String joint, T... array) {
		return join(joint, array, 0, array.length);
	}
	
	public static <T> String join(String joint, T[] array, int off) {
		return join(joint, array, off, array.length);
	}
	
	public static <T> String join(String joint, T[] array, int off, int len) {
		StringBuilder builder = new StringBuilder();
		boolean first = true;
		Object item;
	...
	public static int count(String string, int codePoint) {
	...
	public static String removeStringQuotes(String string) {
	...
	public static String repeat(String string, int count, String delimiter) {
	...
	public static String titlize(String string, boolean eachWord) {
		if(string == null || string.isEmpty())
			return string;
		return eachWord
	...
	public static <T> Callable<T> callable(CheckedRunnable runnable) {
		return callable(runnable, null);
	}
	
	public static <T> Callable<T> callable(CheckedRunnable runnable, T defaultValue) {
	...
	public static <K, V> Map<K, V> mergeNew(Map<K, V> a, Map<K, V> b) {
		return mergeNew(a, b, HashMap::new);
	}
	
	public static <K, V> Map<K, V> mergeNew(Map<K, V> a, Map<K, V> b, Supplier<Map<K, V>> supplier) {
		if((supplier == null))
	...
	public static <T> Collection<T> mergeNew(Collection<T> a, Collection<T> b) {
	...
	public static <A, B, C> Iterator<? extends C> zipIterator(Stream<? extends A> a, Stream<? extends B> b,
                                                              BiFunction<? super A, ? super B, ? extends C> zipper) {
		return zip(a, b, zipper).iterator();
	}
	
	public static <A, B, C> Iterable<? extends C> zipIterable(Stream<? extends A> a, Stream<? extends B> b,
                                                              BiFunction<? super A, ? super B, ? extends C> zipper) {
	...
	public static void dumpThreads(Consumer<Thread> threadConsumer, Consumer<StackTraceElement> elementConsumer) {
	...
	public static Throwable throwable(Class<? extends Throwable> clazz, String message, Object... args) {
	...
	public static void throwISE(String message, Object... args) {
	...
	@SuppressWarnings("unchecked")
	public static <T> List<T> streamToList(Stream<? super T> stream) {
		return stream.collect(Collectors.mapping((m) -> (T) m, Collectors.toList()));
	}
	
	@SuppressWarnings("unchecked")
	public static <T> List<T> streamToUnmodifiableList(Stream<? super T> stream) {
		return stream.collect(Collectors.mapping((m) -> (T) m, Collectors.toUnmodifiableList()));
	}
	
	public static <T, I extends Iterable<T>> I nonNullItems(I iterable) {
		if(iterable == null) return null;
		for(T item : iterable) if(item == null) return null;
		return iterable;
	}
	
	public static <T> T[] nonNullContent(T[] array) {
		if(array == null) return null;
		for(T item : array) if(item == null) return null;
		return array;
	}
	
	public static <T, C extends Collection<T>> C nonNullContent(C collection) {
		return nonNullItems(collection);
	}
	
	public static <T> boolean contains(Collection<T> collection, T value, Function<T, T> mapper) {
		return mapper != null
	...
	public static <T> int indexOf(T needle, T[] haystack, BiFunction<T, T, Boolean> equals) {
	...
	public static <T> int compareIndex(T a, T b, T[] all) {
		return Integer.compare(indexOf(a, all), indexOf(b, all));
	}
	
	public static <T> int compareIndex(T a, T b, List<T> all) {
		return Integer.compare(all.indexOf(a), all.indexOf(b));
	}
	
	public static <T> int compareIndex(T a, T b, T[] all, BiFunction<T, T, Boolean> equals) {
	...
	public static <T> T primitive(T value, Class<? extends T> clazz) {
	...
	public static int compare(Object... items) {
		if((items.length & 1) != 0)
	...
	public static <T> boolean equalsNoOrder(List<? extends T> a, List<? extends T> b) {
	...
	public static String transformEachWord(String string, BiFunction<String, Integer, String> transformer) {
	...
	public static int compareCodePoint(int a, int b) {
		return Integer.compare(a, b);
	}
	
	public static int compareCodePointIgnoreCase(int a, int b) {
		if(a == b) return 0;
	...
	public static int compareNaturalIgnoreCase(String a, String b) {
		return compareNatural(a, b, true);
	}
	
	private static int compareNatural(String a, String b, boolean ignoreCase) {
	...
	public static boolean isSimpleDateTime(String string) {
		return Detections.OfSimpleDateTime.matches(string);
	}
	
	private static int compareSimpleDateTime(Matcher ma, Matcher mb) {
	...
	public static int compareSimpleDateTime(String a, String b) {
		Matcher ma, mb;
	...
	public static int compareNaturalWithDateTime(String a, String b) {
		return compareNaturalWithDateTime(a, b, false);
	}
	
	public static int compareNaturalWithDateTimeIgnoreCase(String a, String b) {
		return compareNaturalWithDateTime(a, b, true);
	}
	
	private static int compareNaturalWithDateTime(String a, String b, boolean ignoreCase) {
		Matcher ma, mb;
		if((ma = Detections.OfSimpleDateTime.match(a)).find() && ma.start() == 0
				&& (mb = Detections.OfSimpleDateTime.match(b)).find() && mb.start() == 0) {
			int cmp = compareSimpleDateTime(ma, mb);
	...
	public static StringBuilder utf16StringBuilder() {
		return OfStringBuilder.newUTF16();
	}
	
	public static StringBuilder utf16StringBuilder(int capacity) {
		return OfStringBuilder.newUTF16(capacity);
	}
	
	public static <T> String toString(List<T> list, Function<T, String> mapper) {
		StringBuilder builder = new StringBuilder();
	...
	private static final class Internal {
		...
		public static void throwRuntimeException(Exception exception,
	...
	private static final class OfStringBuilder {
		...
		private static StringBuilder newInternalBuilder() {
		...
		public static StringBuilder newUTF16() {
			return newUTF16(DEFAULT_CAPACITY);
		}
		
		public static StringBuilder newUTF16(int capacity) {
	...
	private static final class Detections {
		...
		private static Matcher matcher(String key, Regex regex) {
			return ThreadLocals.getOrSupply(key, MatcherPool::new).create(regex);
		}
		
		private static void dispose(String key, Matcher matcher) {
		...
		private static final class OfSimpleDateTime {
			...
			public static void unmatch(Matcher matcher) {
				dispose(KEY, matcher);
			}
		}
		...
		private static final class MatcherPool {
			...
			private Matcher emptyMatcher(Regex regex) {
		...
		private static final class ThreadLocals {
			...
			public static <T> T getOrSupply(String key, Supplier<T> supplier) {
	...
	public static final class OfString {
		...
		private static Collator newCollatorNormalized() {
			Collator collator = Collator.getInstance();
			collator.setDecomposition(Collator.CANONICAL_DECOMPOSITION);
			return collator;
		}
		
		private static int intCharOrSign(int c, int radix) {
			int v;
		...
		private static int intCharOrSignOrRoman(int c, int radix) {
			int v;
			return (v = Character.digit(c, radix)) != -1
						|| (v = c) == '-' || (v = c) == '+'
		...
		private static int intChar(int c, int radix) {
			int v;
			return (v = Character.digit(c, radix)) != -1 ? v : -1;
		}
		
		private static int intCharOrRoman(int c, int radix) {
			int v;
			return (v = Character.digit(c, radix)) != -1
						|| (v = ROMAN_NUMERALS.getOrDefault((char) c, -1)) != -1
					? v : -1;
		...
		private static int intCharRadix(int c, int radix) {
			int v;
			return (v = Character.digit(c, radix)) != -1 ? v : -1;
		}
		
		private static int intCharRoman(int c) {
			return ROMAN_NUMERALS.getOrDefault((char) c, -1);
		}
		
		private static boolean isIntSign(int c) {
			return c == '-' || c == '+';
		}
		
		private static boolean isIntRoman(int c) {
			return ROMAN_NUMERALS.containsKey((char) c);
		}
		
		private static Integer asInteger(String string, int radix, boolean romanNumerals, int start, int end,
		...
		private static int checkInt(Integer v) {
			if(v == null) {
		...
		public static Comparator<String> comparingNormalized() {
			return OfString::compareNormalized;
		}
		
		public static <T> Comparator<T> comparingNormalized(Function<T, String> extractor) {
		...
		public static String concat(String separator, Predicate<String> include, String... values) {
		...
		public static String[] split(String string, int limit, int start) {
			return split(string, limit, start, string.length());
		}
		
		public static String[] split(String string, int limit, int start, int end) {
			if(string == null) {
				throw new NullPointerException();
			}
		...
		public static String[] split(String string, String separator, int limit, int start) {
			return split(string, separator, limit, start, string.length());
		}
		
		public static String[] split(String string, String separator, int limit, int start, int end) {
		...
		public static Integer asInteger(String string, int radix) {
			return asInteger(string, radix, true, 0, string.length(), null);
		}
		
		public static Integer asInteger(String string, int radix, boolean romanNumerals) {
			return asInteger(string, radix, romanNumerals, 0, string.length(), null);
		}
		
		public static Integer asInteger(String string, int start, int end) {
			return asInteger(string, 10, true, start, end, null);
		}
		
		public static Integer asInteger(String string, int radix, int start, int end) {
			return asInteger(string, radix, true, start, end, null);
		}
		
		public static Integer asInteger(String string, int radix, boolean romanNumerals, int start, int end) {
		...
		public static Integer asIntegerOrDefault(String string, int radix, Integer defaultValue) {
			return asInteger(string, radix, true, 0, string.length(), defaultValue);
		}
		
		public static Integer asIntegerOrDefault(String string, int radix, boolean romanNumerals,
                                                 Integer defaultValue) {
			return asInteger(string, radix, romanNumerals, 0, string.length(), defaultValue);
		}
		
		public static Integer asIntegerOrDefault(String string, int start, int end, Integer defaultValue) {
			return asInteger(string, 10, true, start, end, defaultValue);
		}
		
		public static Integer asIntegerOrDefault(String string, int radix, int start, int end,
                                                 Integer defaultValue) {
			return asInteger(string, radix, true, start, end, defaultValue);
		}
		
		public static Integer asIntegerOrDefault(String string, int radix, boolean romanNumerals, int start,
		...
		public static int asInt(String string, int radix) {
			return checkInt(asInteger(string, radix, true, 0, string.length(), null));
		}
		
		public static int asInt(String string, int radix, boolean romanNumerals) {
			return checkInt(asInteger(string, radix, romanNumerals, 0, string.length(), null));
		}
		
		public static int asInt(String string, int start, int end) {
			return checkInt(asInteger(string, 10, true, start, end, null));
		}
		
		public static int asInt(String string, int radix, int start, int end) {
			return checkInt(asInteger(string, radix, true, start, end, null));
		}
		
		public static int asInt(String string, int radix, boolean romanNumerals, int start, int end) {
			return checkInt(asInteger(string, radix, romanNumerals, start, end, null));
		}
		
		public static int asIntOrDefault(String string, int defaultValue) {
			return asInteger(string, 10, true, 0, string.length(), defaultValue);
		}
		
		public static int asIntOrDefault(String string, int radix, int defaultValue) {
			return asInteger(string, radix, true, 0, string.length(), defaultValue);
		}
		
		public static int asIntOrDefault(String string, int radix, boolean romanNumerals, int defaultValue) {
			return asInteger(string, radix, romanNumerals, 0, string.length(), defaultValue);
		}
		
		public static int asIntOrDefault(String string, int start, int end, int defaultValue) {
			return asInteger(string, 10, true, start, end, defaultValue);
		}
		
		public static int asIntOrDefault(String string, int radix, int start, int end, int defaultValue) {
			return asInteger(string, radix, true, start, end, defaultValue);
		}
		
		public static int asIntOrDefault(String string, int radix, boolean romanNumerals, int start, int end,
                                         int defaultValue) {
			return asInteger(string, radix, romanNumerals, start, end, defaultValue);
		}
	}
	...
	public static final class UnicodeEscapeSequence {
		...
		private static int eq(int c, int r) {
		...
		// Returns 1, if c > r, otherwise 0.
		private static int gt(int c, int r) {
			// c >  r, (r - c) >>> 31 = 1
			// c <= r, (r - c) >>> 31 = 0
			return ((r - c) >>> 31);
		}
		...
		private static int in(int c, int a, int b) {
		...
		private static int isHex(int c) {
		...
		//     2, if f(i) == 2,
		//     3, if f(i) == 4,
		private static int dfaIdx(int r) {
			return Integer.lowestOneBit(r) - gt(r, 3);
		}
		
		private static int transition(int s, int c) {
			/* f(s, c) = {
			 *     c == '\\'                                  -> 0,
			 *     c == 'u'                                   -> 1,
			 *     c in [A-Fa-f0-9]                           -> 2,
			...
			return gt0 * dfaTable[s * 3 + (idx - gt0)];
		}
		
		private static void doOperation(StringBuilder input, StringBuilder output, final int len,
		...
		// Since CharSequence does not have codePointAt method, we must copy the method above
		// with the only exception of using a String instead of a StringBuilder as an input.
		private static void doOperation(String input, StringBuilder output, int len, OperationString operation) {
			int end = 0;
		...
		private static void opReplace(StringBuilder output, StringBuilder input, int start, int end) {
			output.appendCodePoint(Integer.parseInt(input, start + 2, end, 16));
		}
		
		private static void opPrefix(StringBuilder output, StringBuilder input, int start, int end, String prefix) {
			output.append(prefix).append(input, start, end);
		}
		
		private static void opReplace(StringBuilder output, String input, int start, int end) {
			output.appendCodePoint(Integer.parseInt(input, start + 2, end, 16));
		}
		
		private static void opPrefix(StringBuilder output, String input, int start, int end, String prefix) {
			output.append(prefix).append(input, start, end);
		}
		
		public static void replace(StringBuilder input, StringBuilder output) {
			final int len = input.length();
			doOperation(input, output, len, UnicodeEscapeSequence::opReplace);
		}
		
		public static void replace(String input, StringBuilder output) {
			final int len = input.length();
			doOperation(input, output, len, UnicodeEscapeSequence::opReplace);
		}
		
		public static String replace(StringBuilder input) {
			final int len = input.length();
			StringBuilder builder = utf16StringBuilder(len);
		...
		public static void prefix(StringBuilder input, StringBuilder output, String prefix) {
			final int len = input.length();
			doOperation(input, output, len, (out, in, start, end) -> opPrefix(out, in, start, end, prefix));
		}
		
		public static void prefix(String input, StringBuilder output, String prefix) {
			final int len = input.length();
			doOperation(input, output, len, (out, in, start, end) -> opPrefix(out, in, start, end, prefix));
		}
		
		public static String prefix(StringBuilder input, String prefix) {
			final int len = input.length();
			StringBuilder builder = utf16StringBuilder(len);
		...
		private interface OperationStringBuilder {
			
			void execute(StringBuilder output, StringBuilder input, int start, int end);
		}
		
		private interface OperationString {
	...
	public static final class Suppress {
		...
		public static <T> Runnable runnable(CheckedRunnable runnable) {
		...
		public static <T> Supplier<Boolean> supplier(CheckedRunnable runnable) {
	...
	public static final class Ignore {
		...
		private static <T> T throwingIgnore(Callable<T> callable, Supplier<T> defaultValueSupplier,
                                            Function<Exception, Exception> exceptionMapper) throws Exception {
			try {
		...
		private static <T> T ignore(Callable<T> callable, Supplier<T> defaultValueSupplier,
                                    Function<Exception, Exception> exceptionMapper) {
			try {
				return callable.call();
			} catch(Exception ex) {
		...
		public static <T> T call(Callable<T> callable, Consumer<Exception> exceptionHandler) {
			return call(callable, Internal.exceptionMapper(exceptionHandler));
		}
		
		public static <T> T call(Callable<T> callable, Function<Exception, Exception> exceptionMapper) {
			return defaultValue(callable, null, exceptionMapper);
		}
		
		public static <T> void callVoid(CheckedRunnable runnable) {
			callVoid(runnable, (Function<Exception, Exception>) null);
		}
		
		public static <T> void callVoid(CheckedRunnable runnable, Consumer<Exception> exceptionHandler) {
		...
		public static <T> boolean callAndCheck(CheckedRunnable runnable) {
			return callAndCheck(runnable, (Function<Exception, Exception>) null);
		}
		
		public static <T> boolean callAndCheck(CheckedRunnable runnable, Consumer<Exception> exceptionHandler) {
		...
		public static <T> T defaultValue(Callable<T> callable, T defaultValue) {
		...
		public static <T> T supplier(Callable<T> callable, Supplier<T> defaultValueSupplier) {
		...
		public static final class Cancellation {
			...
			private static Function<Exception, Exception> exceptionMapper(
                    Consumer<Exception> exceptionHandler) {
			...
			public static <T> T call(Callable<T> callable, Consumer<Exception> exceptionHandler) throws Exception {
			...
			public static <T> void callVoid(CheckedRunnable runnable) throws Exception {
				callVoid(runnable, (Function<Exception, Exception>) null);
			}
			...
			public static <T> boolean callAndCheck(CheckedRunnable runnable) throws Exception {
				return callAndCheck(runnable, (Function<Exception, Exception>) null);
			}
			...
			public static <T> T defaultValue(Callable<T> callable, T defaultValue) throws Exception {
				return defaultValue(callable, defaultValue, (Function<Exception, Exception>) null);
			}
	...
	public static final class OfPath {
		...
		public static Info info(String path) {
			return Info.of(path);
		}
		
		public static Info info(Path path) {
			return Info.of(path);
		}
		...
		public static String dirPath(Path path) {
		...
		public static final class Info {
			...
			private static Info of(Path path, String baseName) {
			...
			private static Info of(String path) {
				return of(null, extractBaseName(path));
			}
			
			private static Info of(Path path) {
	...
	public static final class OfFormat {
		...
		public static String time(double time, TimeUnit unit, boolean alwaysShowMs) {
			return time((long) time, unit, alwaysShowMs);
		}
		
		public static String time(long time, TimeUnit unit, boolean alwaysShowMs) {
			StringBuilder builder = new StringBuilder();
		...
		public static String size(double size, SizeUnit unit, int decimals) {
			long base = unit.base();
	...
	public enum SizeUnit {
		...
		SizeUnit(String abbreviation, long base, long power) {
	...
	public static final class JS {
		...
		public static String extractVariableContent(String script, String varName, int offset) {
			StringBuilder builder = new StringBuilder();
		...
		public static String extractFunctionPararms(String script, String funcName, int offset) {
			StringBuilder builder = new StringBuilder();
		...
		public static String insertArgumentsToString(String statement, Map<String, String> args) {
